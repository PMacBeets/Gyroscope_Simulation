% MAYBE: reconfigure, gyroscope build and set axis to match real gyroscope
% dimensions
...
%x_init = [.01; .01; .01; 0; .0016;.13;.001; 50];           % initial conditions
x_init = [pi/20; pi/20; pi/20;0; 0;0;0;50];    % sample one suggested by
%                                                 tutor with new correct
%                                                 dimensions


tspan = [0 15];                                 % start and finish
...
%% ---------------------------------------------------------------------------
 %% SETUP VIDEO IF REQUIRED
    if VIDEO
        fps = 1/dt;
        MyVideo = VideoWriter('Gyro_animation_fixed','MPEG-4');
...
fprintf("animation recording end\n");
...
% for ode 45
    ...
    % TODO: copy new equations of motion with correct mass and dimensions
