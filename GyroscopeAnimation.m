%{
-------------------------------- Read Me --------------------------------------
    Code by: David Pham, Jake Langton and Tim Robertson

    GyroscopeAnimation utilised equtions of motion from ExtractEquations.m
    and simulates Gyroscope accordingly.

    Adjustable (uncomment & comment out):
    - VIDEO:    video file output can be toggled where 1 = ON and 0 = OFF
    - x_init:   can be swapped with its clockwise variant
    - views:    default viewing angle can be used to compare model to Gyroscope
                recordings
%}
...
%% Constants
    ...
    % and only want integers

    syms t;
    VIDEO = 1;  %toggles video file output
...
% IC's for anti-clockwise (positive rotation)
x_init = [pi/20; pi/11; pi/20;0; pi/4;0;0;800];
...
% Axle components
    axle_R = r;   %axle radius

% outside frame
    oAxle_h = 3;  %length of bottom and top ends
    t2_max = t2_R + t2_r;
...
for i = 1:length(t)
    ...
    % transformation for frame related objects
    R03 = R01*R12*R23;

    %  transformation for rotor related objects
    R04 = R03*R34;
    ...
    surf(t1_x_rot, t1_y_rot, t1_z_rot,t1_z); hold on
    surf(t2_x_rot, t2_y_rot, t2_z_rot,t2_z); hold on
...
% constants for equations of motion, units in m and kg
    m_r = 0.103;     %rotor mass
%   m_f = 0.023;
    rho = 3037;      %frame density (calculated)
    R_i = 0.035;     %rotor radius
    R_0 = 0.04;      %torus major radius
    r = 0.002;       %cross section radius (torus and cylinder)
    H = 0.1;         %top to bottom
    L = .5 * H;      %bottom to centre
    h = 0.01;        %total thickness of rotor
    g = 9.81;        %gravity
